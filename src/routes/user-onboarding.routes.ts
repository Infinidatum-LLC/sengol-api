...
async function getOnboardingStatus(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    const result = await query(
      `SELECT "id", "email", "emailVerified", "eulaAccepted", "onboardingCompleted", "onboardingCompletedAt"
       FROM "User"
       WHERE "id" = $1 LIMIT 1`,
      [userId]
    )
    ...
    const requiresEulaAcceptance = !user.eulaAccepted
...
async function completeOnboarding(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    if (body.emailVerified !== undefined) {
      ...
      updateValues.push(body.emailVerified)
      paramIndex++
    }
